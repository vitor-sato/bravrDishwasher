...
/*  07/25/2018(VitorSE): Addition of error treatment and pause system         */
...
/*  08/03/2018(VitorSE): Review of the continue_cycle                         */
/*                                                                            */
/*  Todo:                                                                     */
/*    - Measure tankCapacity, timeRegeneration, timeVidange;                  */
/*    - Check operation of cycle pump and diverter motor;                     */
/*                                                                            */
/******************************************************************************/
...
int count1_Sec;                               // Seconds counter of timer 1
int count1_Min;                               // Minutes counter of timer 1
int count3_Sec;                               // Seconds counter of timer 3
int count3_Min;                               // Minutes counter of timer 3
int timeToFill;                               // Necessary time to fill the tank [s]
// State's variables
...
bool cuveFilled;                            // Indicates if the tank if filled with water
bool cntrlDiverter1;                        // Actual state of the pins which controle the direction 
bool cntrlDiverter2;                        // of the water flux
bool flagTemperature;                       // Set to HIGH when the continue cycle must use the Temperature Phase to heat the water (final rinse)
bool flagTimer1;                            // Indicates if the timer1 is being used or not
...
/******************************************************************************/
void pause_cycle() {
  tempTimer1 = TCNT1;                               // Save current counting on the timers
  tempTimer3 = TCNT3;                               
  ...
  int tempCount1Sec = count1_Sec;                   // Save the current counting on the timers
  ...
  count1_Sec = tempCount1Sec;                         // Restore the countings on the timers
...
void continue_cycle() {
  ...
  // Treats the special case of the heating, since the cycle must be stopped for so long that the temperature of the water
  // is not the same when the cycle was running
  switch (lavagePhase) {
    case 3:                                                               // Mainwash
      if(temperatureRead != 0.0 and tempChauffage == LOW){                // Checks if the water temperature had to be in a certain temperature
        ...
        do{
          temperatureRead = s.mesure_temperature();                       // Constantly updates the water temperature until the cycle continue
          Serial.println("ºC");
          delay(2000);
        }while(doorState == LOW);
        
        if(temperatureRead < temperaturePhase){                           // If the actual temperature is lower than it should be
            Serial.println("The heating was turned on again to reach the needed temperature");
            a.chauffage_on();                                             // Heats the water again before restarting the cycle 
            do{                                                           
              temperatureRead = s.mesure_temperature();
              Serial.print(temperatureRead);
              Serial.println("ºC");
      ...
      break;
    case 5:                                                           // Final rinse
      if(flagTemperature == HIGH){                                    // Based on the flag, it will know in each temperature the water should be
          tempTemperature = temperaturePhase;
        }
      else{
          tempTemperature = temperatureInitial;
      }
      // Checks if the water temperature had to be in a certain temperature
      if((temperatureRead != 0.0 and tempChauffage == LOW) or (temperatureRead != 0.0 and tempDoseur == HIGH)){ 
          do{
            temperatureRead = s.mesure_temperature();                 // Constantly updates the water temperature until the cycle continue
            Serial.print(temperatureRead);
            Serial.println("ºC");
          ...
          if(temperatureRead < tempTemperature){                      // If the actual temperature is lower than it should be
              Serial.println("The heating was turned on again to reach the needed temperature");
              a.chauffage_on();                                       // Heats the water again before restarting the cycle 
              do{                                                           
                temperatureRead = s.mesure_temperature();
  ...
  // Restore the state of the timers
  if(flagTimer1 == HIGH){
    if(count1_Min == 0){
...
/******************************************************************************/
/*                                                                            */
/*  Routine responsible to manage the interruption of the timer 1 in order to */
/*  allow the system to add time delays in the program. It will last the      */ 
/*  amount of time indicated in the routine parameter in seconds.             */
/*                                                                            */
/******************************************************************************/
void wait_sec(int seconds){
    ...
    do{
      count = count1_Sec;
      delay(50);
    }while(count < seconds and doorState == HIGH);
...
/*  Routine responsible to manage the interruption of the timer 1 in order to */
/*  allow the system to add time delays in the program. It will last the      */ 
/*  amount of time indicated in the routine parameter in minutes.             */
/*                                                                            */
/******************************************************************************/
...
/******************************************************************************/
/*                                                                            */
/*  Routine responsible to to execute all the necessary steps in order to     */
/*  to completely fill the tank. It receives as a parameter, how much time it */ 
/*  should let the filling valve on.                                          */
/*                                                                            */
/******************************************************************************/
...
/******************************************************************************/
/*                                                                            */
/*  Routine dedicated to deactivate all the components of the dishwasher.     */
/*                                                                            */
/******************************************************************************/
void turn_off(){
